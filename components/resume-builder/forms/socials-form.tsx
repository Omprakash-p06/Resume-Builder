...
export function SocialsForm({ data, onChange, isDark }: SocialsFormProps) {
  const handleAdd = () => {
    onChange([
      ...data,
      {
  ...
  const handleUpdate = (id: string, field: string, value: string) => {
    onChange(
      data.map((social) => (social.id === id ? { ...social, [field]: value } : social))
    )
  }

  const handleDelete = (id: string) => {
    onChange(data.filter((social) => social.id !== id))
  }
